...
class campionato:
    ...
    def __dbOpen(self, autocommit=False):
        ...
        '''
        return pymysql.connect("localhost","root","root",\
                               "esame",autocommit=autocommit)    
        ...
        with self.__dbOpen() as cursor:
            for squadra in squadre:
                try:
                    ...
                    return campionato.ERR_SEL, squadra, "errore nella select squadra"
                if nlines == 0:
                    try:
                        ...
                        cursor.connection.rollback()
                        return campionato.ERR_INS, squadra, query2
                    try:
                        cursor.execute(query, squadra) 
                    except:
                        return campionato.ERR_SEL, squadra, "errore nella select dopo inserimento della squadra"               
                idSquadre[squadra] = cursor.fetchone()[0]   
        return campionato.QUERY_OK, idSquadre, "Inserimento squadre effettuato"
    ...
    def __nuovoCamp(self, nome):
        ...
        retcode, ids, info = self.__getIdSquadre(self.__squadre)
        if retcode != campionato.QUERY_OK:
            return retcode, ids, info
        ...
        for numGiornata, giornata in enumerate(giornate):
            for partita in giornata['matches']:
                ...
                if retcode  != campionato.QUERY_OK:
                    return retcode, query2
                idGara = result[0]
                retcode = self.__insQuery(query3, (idGara, \
                                            partita['score1'], \
                                            partita['score2']))
                if retcode != campionato.QUERY_OK:
                    return retcode, query3
        ...
        if retcode  != campionato.QUERY_OK:
            return retcode, idcamp, "prima vista"
        ...
        if retcode  != campionato.QUERY_OK:
            return retcode, query, "seconda vista"
